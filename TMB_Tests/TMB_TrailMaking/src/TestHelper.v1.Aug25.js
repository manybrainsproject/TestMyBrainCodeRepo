/*
 ...
 Copyright 2025 The Many Brains Project, Inc. and McLean Hospital.
...
{
    /*
      Globally disables double-tap zoom on touch devices.
...
function tmbSubmitToServer(tmbData,tmbScore,tmbOutcomes,tmbAction,debug)
{
    /*
     Submits data, score and outcomes to the tmb server.
     To test connectivity, a HEAD request is made before form submission: a
     warning appears on failure, providing an option to re-submit.

     Parameters:

        tmbData:       An object or array containing trial by trial results
                       (this will be JSON stringyfied before transmission).
        tmbScore:      The participant's score in the test.
        tmbOutcomes:   An object containing outcome variables in the form of
                       key:value pairs, or the string 'null'.
                       (This will be JSON stringyfied before transmission).
        tmbAction:     The URL to submit the data. Optional, defaults to '/run'.
        debug:         Boolian [true,false], default false; when set true,
                       property submitFailure is added to outcomes.

     Usage:

        tmbSubmitToServer(data,score,outcomes,'./run.php',true);

    */

    tmbAction = tmbAction ? tmbAction : '/run';
    if(debug && tmbOutcomes && !tmbOutcomes.hasOwnProperty('submitFailure'))
        tmbOutcomes.submitFailure = false;

    var lang = document.documentElement.lang;
    var dict = {'en': {'msg': '<h3>Network error</h3>'+
                              'Please check your internet connection,<br>'+
                              'then click the button below to proceed.<br><br>',
                       'btn': 'Click to proceed'},
                'fr': {'msg': '<h3>Erreur de réseau</h3>'+
                              'Veuillez vérifier votre connexion internet,<br>'+
                              'puis cliquez sur le bouton ci-dessous<br>'+
                              'pour poursuivre.<br><br>',
                       'btn': 'Cliquez pour poursuivre'}};
    var msg = dict.hasOwnProperty(lang) ? dict[lang].msg : dict['en'].msg;
    var btn = dict.hasOwnProperty(lang) ? dict[lang].btn : dict['en'].btn;
    ...
    var xhrErrorHandler = function()
    {
        showAlert(msg,
            ...
            function ()
            {
                getID('alertButton').onclick = function(){return false};
                getID('alertBox').style.display = 'none';
                setTimeout(function()
                {
                    if(debug) tmbOutcomes.submitFailure = true;
                    tmbSubmitToServer(tmbData,tmbScore,
                                      tmbOutcomes,tmbAction,debug);
                },500);
            },
            '20pt');
    };
    ...
    xhr.onload = function()
    {
        ...
        // create a form with 3 hidden inputs: data, score and outcomes
...
function langDetect()
{
    /*
     ...
     Detects whether the page is being translated by the browser.

     Usage:

        - Set the root html element:
            <html lang="en" tmblang="en">

        - To retrieve the original language:
            langDetect().tmbLanguage

        - To retrieve the translated language:
            langDetect().translatedTo

     Notes:

      langDetect().translatedTo assumes the following values:

      - "unknown":  the "lang" and/or "tmblang" attributes are not defined in
                    the root html element, or translation has been detected, but
                    the language cannot be determined.
      - "none":     the page's laguage appears to match the original language.
      - lang tag:   the page language, as one of the language tags specified in
                    RFC 5646, when different than the original language.

      As of 01/2025, the translation behavior of browsers is as follows.

      Chrome:   "lang" is set to the translated language tag. The class
                "translated-ltr" or "translated-rtl" is added to the root html
                element.

      Firefox:  "lang" is set to the translated language tag. Older versions
                using the Bergamot extension added the attribute
                "x-bergamot-translated" to all visible page elements.

      Safari:   "lang" is set to the translated language tag.

      Edge:     the attribute "_msttexthash" is added to all translated elements,
                its value being a hash of the translated text.

      Yandex:   all translated texts are wrapped in a custom "ya-tr-span"
                element.
    ...
    */
    ...
    var browserLang = document.documentElement.lang;
    var tmbLang =  document.querySelector("html").attributes["tmblang"] ?
        document.querySelector("html").attributes["tmblang"].textContent :
        "";
    var edgeTrans = document.querySelector("*[_msttexthash]");
    var googleTrans = document.querySelector("html.translated-ltr, html.translated-rtl");
    var yandexTrans = document.querySelector("ya-tr-span");
    var FFextTrans = document.querySelector("*[x-bergamot-translated]");
    var translatedTo;
    if(!browserLang || !tmbLang)
    {
        tmbLang = 'unknown';
        translatedTo = 'unknown';
    }
    else if(!edgeTrans && !googleTrans && !yandexTrans && !FFextTrans &&
        browserLang === tmbLang)
    {
        translatedTo = 'none';
    }
    else if(browserLang !== tmbLang)
    {
        translatedTo = browserLang;
    }
    else translatedTo = 'unknown';
